...
用法：
    python scripts/batch_eval.py --csv data/real_cases.csv --limit 60 --out results.csv
评估指标：
  - 违规检出：以 comment 非空为人工违规代理标签，统计 P/R/F1。
  - 类目准确率：预测 scene_category 是否落入人工标签归一化后的可接受类目集。
  - 涉诈准确率：人工标签可判定涉诈时，比对 is_fraud。
支持并发（--workers）与快速/工具两种模式（--tools 开启完整 agentic loop）。
"""
...
def main() -> int:
    ...
    def run_one(case):
        res = agent.inspect(case.content, data_id=case.data_id, use_tools=use_tools)
        return case, res
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        ...
        for fut in as_completed(futures):
            ...
            if args.verbose or done % 10 == 0:
                print(f"  [{done}/{len(items)}] {case.data_id} 人工『{case.comment}』 -> {res.label}")
    ...
    _report(rows, store, elapsed)
    print(f"结果已写入：{out_path}")
    return 0


def _report(rows, store, elapsed):
    tp = fp = fn = tn = 0
    ...
    print("=" * 56)
    print(f"耗时：{elapsed:.1f}s | 平均 {elapsed / max(len(rows),1):.2f}s/条")
