...
python3 scripts/inspect_text.py -v -f call.txt
```

`LLM_BASE_URL` 兼容任意 OpenAI 协议服务：OpenAI、Claude 兼容网关、通义千问 DashScope 兼容模式、DeepSeek、GLM 等。开启后即由模型驱动 agent loop，自主调用工具调查后给出结论。
