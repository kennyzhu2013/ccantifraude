...
## 9. 对客最新复核规范同步（rules.json v1.2.1）

相对 V1.1，附件决策表带来的关键口径更新已写入 `knowledge/latest_decision_table.md` / `.csv` 与 `rules.json`（已按损坏原件 GBK 锚点逐字校对）：

| 变更 | 口径 |
|---|---|
| ab贷 | 由贷款相关高风险违规 **升级为涉诈**（`贷款相关-ab贷`） |
| 引导用户添加私人微信 | **新增涉诈**：平台核资后引导加下款/放款经理私人微信 |
| 路费诈骗 | **新增涉诈**：招聘+仓库/路线+入职注册费（原文金额：800 元 / 第一单 500） |
| 企业营销细分 | 代开发票/注册注销年报代办收费=中风险；培训提前收费=低风险；人工获客/AI客服=合规 |
| 法律服务 / 催收 | 维权成功后收费、冒充公检法催收 → 高风险 |

> 上传 xlsx 与首次损坏件字节级相同，无法直接解析；校对以同批 CSV 存活 GBK 锚点 + V1.1 重叠条文约束还原。演进补充（验证码转发、降息套现、设备租赁招商变体）仍保留在 `rules.json`，并在决策表中与原文条目区分标注。
