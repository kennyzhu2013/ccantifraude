...
## 10. LLM 质量天花板验证（deepseek-v4-flash 快速单轮）

接入 DeepSeek 后在 `data/eval_fresh_cases.csv`（28 条全新手写用例：21 风险 + 7 正常，覆盖全部 16 类目，
与规范原文/RAG 判例库零重叠，无检索泄漏）上的回归结果：

| 指标 | 启发式兜底 | LLM 快速单轮 |
|---|---|---|
| 违规检出（21 违规 + 7 正常） | R=0.889，有漏判 | **R=1.000，P=1.000（零误报）** |
| 类目准确率 | 14/24 | **21/21 = 1.000** |
| 涉诈判定 | 8/10 | **10/10 = 1.000** |

本轮验证同时修复的问题（复现方式：`QC_CACHE_PATH= python scripts/batch_eval.py --csv data/eval_fresh_cases.csv`）：

1. **few-shot 照抄**：模型偶发把检索到的相似判例（如证券引流）的结论套到待质检文本（如航班改签）上，
   `prompts.py` 快速模式追加『结论只能针对待质检文本、evidence_quotes 必须出自待质检原文』硬约束；
2. **类目/子类目倒置**：模型偶发把真实类目写进 `scene_subtype`（如『其他/个体工商户年报补录收费』），
   `agent._finalize` 增加确定性归一化提升 subtype；
3. **规则碰撞**：『人工获客/人工智能客服（合规）』被套用到收费 AI 获客推广上（已改写措辞区分）；
   ab贷 案例因语气平和被判正常（已加反漏判护栏）；保健品『免费领+加微信』被就高误判引流
   （已明确加微信是该套路组成环节，整体判商品推销·中风险）；
4. **评估口径**：人工标签明确『正常/合规』的样本不再参与类目/涉诈准确率（此前会因『催收』等
   关键词制造伪冲突）；规范类目全名在标签归一化中最高优先。
